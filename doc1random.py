...
def Trail(batch_size, initailize_percent):
    ...
    while knownnum < total:
        ...
        score_correct_mean = np.array(score_correct).mean()
        score_false_mean = np.array(score_false).mean()
        entropy_correct_mean = np.array(entropy_correct).mean()
        entropy_false_mean = np.array(entropy_false).mean()
        '''
        ...
        accuracy[knownnum] = correct/count
        scores[knownnum] = [score_correct_mean, score_false_mean]
        selections = Make_Selections_score(size, local_prediction)#RANDOM

        batch_score = 0
        for selection in selections:
            row = selection[0]
            col = selection[1]
            batch_score += selection[3]

            currdata[row][col] = ground_truth[row][col]
        batch_score /= len(selections)
        batch_scores[knownnum] = batch_score
        knownnum = CountKnownNum(currdata)
...
def Trail_Random(batch_size, initailize_percent):
    ...
    while knownnum < total:
        ...
        accuracy[knownnum] = correct/count
        scores[knownnum] = [score_correct_mean, score_false_mean]
        selections = Make_Selections_random(size, local_prediction)#RANDOM
...
def main():
    if __name__ == "__main__":
        ...
        for k in range(num_of_trails):
            ...
            accuracy_in_one_trailR, score_in_one_trailR, batch_scores_in_one_trailR = Trail(batch_size, initialize_percent)
            for percent in accuracy_in_one_trail:
                if percent in accuracy_score:
                    ...
                    accuracy_score[percent] = accuracy_in_one_trail[percent]

            for num in score_in_one_trail:
                if num in uncertainty_score:
                    uncertainty_score[num][0] = uncertainty_score[num][0] + score_in_one_trail[num][0]
                    uncertainty_score[num][1] = uncertainty_score[num][1] + score_in_one_trail[num][1]
                else:
                    uncertainty_score[num] = score_in_one_trail[num]

            for num in batch_scores_in_one_trail:
                if num in batch_scores:
                    batch_scores[num] = batch_scores[num] + batch_scores_in_one_trail[num]
                else:
                    batch_scores[num] = batch_scores_in_one_trail[num]
#----------------------------------------------------------------------------------------------------------------
            for percent in accuracy_in_one_trailR:
                if percent in accuracy_scoreR:
                    accuracy_scoreR[percent] = accuracy_scoreR[percent]+accuracy_in_one_trailR[percent]
                else:
                    accuracy_scoreR[percent] = accuracy_in_one_trailR[percent]

            for num in score_in_one_trailR:
                if num in uncertainty_scoreR:
                    uncertainty_scoreR[num][0] = uncertainty_scoreR[num][0] + score_in_one_trailR[num][0]
                    uncertainty_scoreR[num][1] = uncertainty_scoreR[num][1] + score_in_one_trailR[num][1]
                else:
                    uncertainty_scoreR[num] = score_in_one_trailR[num]

            for num in batch_scores_in_one_trailR:
                if num in batch_scoresR:
                    batch_scoresR[num] = batch_scoresR[num] + batch_scores_in_one_trailR[num]
                else:
                    batch_scoresR[num] = batch_scores_in_one_trailR[num]
        ...
        for key in uncertainty_score:
            uncertainty_score[key][0] = uncertainty_score[num][0]/num_of_trails
            uncertainty_score[key][0] = uncertainty_score[num][0]/num_of_trails

        for key in batch_scores:
            batch_scores[key] = batch_scores[key]/num_of_trails

            #-------------------------------------------------------------------------
        for key in accuracy_scoreR:
            accuracy_scoreR[key] = accuracy_scoreR[key]/num_of_trails

        for key in uncertainty_scoreR:
            uncertainty_scoreR[key][0] = uncertainty_scoreR[num][0]/num_of_trails
            uncertainty_scoreR[key][0] = uncertainty_scoreR[num][0]/num_of_trails

        for key in batch_scoresR:
            batch_scoresR[key] = batch_scoresR[key]/num_of_trails
        ...
        print("score: ", uncertainty_score)
        print("batch score: ", batch_scores)

        print("RANDOM!")
        print("accuracy: ", accuracy_scoreR)
        print("score: ", uncertainty_scoreR)
        print("batch score: ", batch_scoresR)

        plt.plot(percent_score, accuracy_score.values(), "-", label="uncertainy: # of observed samples")
        plt.plot(percent_score, accuracy_scoreR.values(), "-", label="uncertainy: random")
