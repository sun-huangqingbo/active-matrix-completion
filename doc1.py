...
def Calculate_TrueFalsth_Ratio(prediction, label):
    ratio = 1
    true_score = prediction[2][label]
    for key in prediction[2]:
        if key != label:
            ratio *= ((1 + true_score)/(prediction[2][key] + 1))
    ...
    while knownnum <  total:
        ...
        for prediction in predictions:
            ...
            if ground_truth[prediction[0]][prediction[1]] == maxlabel:
                correct+=1
                score_correct.append(TF_ratio)
                #entropy_correct.append(entropy)
            else:
                score_false.append(TF_ratio)
...
#   unify the result
        ...
        '''
        #-----------------------------------------------------------------------------
        #convert batch axis into percent
