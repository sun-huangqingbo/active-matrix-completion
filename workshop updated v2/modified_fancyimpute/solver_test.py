...
from common_test import generate_random_column_samples
...
class Solver(object):
    ...
    def fill(
        ...
        if fill_method not in ("zero", "mean", "median", "min", "random", "half"):
            raise ValueError("Invalid fill method: '%s'" % (fill_method))
        elif fill_method == "zero":
            # replace NaN's with 0
            X[missing_mask] = 0
        elif fill_method == "half":
            # replace NaN's with 0.5
            X[missing_mask] = 0.5
    ...
    def prepare_input_data(self, X):
        ...
        X = check_array(X, force_all_finite=False, dtype='f')
