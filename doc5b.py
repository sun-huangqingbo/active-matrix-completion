...
#random
    ...
    while knownnum < total:
        ...
        selections = Make_Selections_score2(0, predictions) #score
        for p in selections:
    ...
    while knownnum < total:
        ...
        selections = Make_Selections_score2(size, predictions) #score
        for p in selections:
            predictions.remove(p)
        for p in predictions:
            p[3] = p[4]
    ...
    filename = r'test(score+entropy)/uni' + str(uniqueness) + 'res' + str(responsiveness)+ 'score&entropy test (no threshold)'  + '60ptps.csv'
    result = []
