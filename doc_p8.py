...
def Predict_One_Col2(col, currdata, threshold):
    unknown_rows = []
    community = Find_Max_community(col, currdata, threshold)
    predictions = []
    ...
    for row in unknown_rows:
        ...
        for col2 in community:
            if currdata[row][col2] != 'u':
                label = currdata[row][col2]
                if label in prediction:
                    prediction[label] += Count_Col_Score(col, col2, currdata)
                else:
                    prediction[label] = Count_Col_Score(col, col2, currdata)
        if len(prediction) != 0:
    ...
    if len(unknown_rows) != 0:
        for row in unknown_rows:
            cols = Find_remain_col(row, col, len(community), currdata)
            prediction = {}
            for col2 in cols:
                if currdata[row][col2] != 'u':
                    label = currdata[row][col2]
...
#Random
    ...
    #plt.plot(accusR.keys(), accusR.values(), '^-', label = 'random')
    #plt.legend(loc='best')
    #plt.show()
