...
#active learning
    ...
    while knownnum < total:
        ...
        for key in prediction_temp:
            ...
            #TF_ratio = Calculate_TrueFalsth_Ratio(prediction_temp[key], maxlabel)
            if dictp != -2:
                bias = sum(dictp.values())
                if bias == 0:
                    for key in dictp:
                        dictp[key] = 1/len(dictp)
                else:
                    for key in dictp:
                        dictp[key] /= bias
                ...
                # for key in dictp:
                #     dictp[key] *= k
                temp = list(dictp.values())
                uncertain_score = entropy(temp, base=2)
            else: uncertain_score = math.log2(32)
...
def main():
    ...
    Trail(0.4, 0.8, "score", args.phenotype)
