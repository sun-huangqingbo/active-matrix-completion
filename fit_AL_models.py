...
def main(u, r, noise, tree):
    ground_truth = Generating_Truth(100, u, r, 32)
    ground_truth = AddNoise(ground_truth, noise, 32)
    current_mtx = Initialize_Data(ground_truth, 100)
    est_resp = estimate_resp(current_mtx)
    est_uniq = estimate_uniq(current_mtx)
    ...
    while known < total:
        ...
        predict = tree.predict([[est_uniq, est_resp, obs, p_num]])
        if predict[0] == 1:
        ...
        known = CountKnownNum(current_mtx)
    filename = 'adaptive_results.csv'
    result = []
    result.append([u, r, noise])
...
if __name__ == "__main__":
    np.random.seed(0)
    file = r"active_learning/IBC_active_training_data_v2.csv"
    file2 = r"active_learning/si_active_training_data.csv"
    ...
    tree = DecisionTreeClassifier(random_state=0, max_depth=4, criterion="entropy").fit(X_all, y_all)
...
dot_data = sklearn.tree.plot_tree(tree,  
                                feature_names=["responsiveness", "observation", "n_phenotypes"],  
                                class_names=["SOFT IMPUTE-Based", "IBC"],
                                filled=True)

# Draw graph
fig.savefig("decistion_tree_active_v3.png")
